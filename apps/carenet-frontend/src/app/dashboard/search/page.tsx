...
export default function SearchPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiClient.post("/documents/search", searchArgs);
    ...
    <>
      ...
      <Row gutter={24}>
        <Col>
          <Card
            title="תוצאות"
            bodyStyle={{ maxHeight: 500, overflowY: "auto", direction: "rtl" }}
            bordered={true}
          >
            {results.length ? (
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        <Button
          htmlType="submit"
          style={{ background: "#001529", color: "white", width: 250 }}
...
const ChipsResultsComponent: React.FC<ChipsResultsComponentProps> = ({
  result,
}) => {
  return (
    <div
      className="flex flex-wrap space-y-2 space-x-2 large-font-padding"
      style={{ width: 300 }}
    >
      {result.email && (
        <div>
          <Tag color="#2db7f5" className="large-font-padding">
            {result.email}
          </Tag>{" "}
        </div>
      )}
    ...
    <div>
      {results.map((result, index) => (
        <Card
          ...
          key={index}
          title={<div style={{ fontSize: "18px" }}>{result.title}</div>}
          style={{ margin: "10px 0" }}
        >
